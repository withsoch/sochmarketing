...
// lg and up (clearing the sticky header), and stacks above the article below
// that. Entries come from the post's own `##` headings - see getHeadings in
// lib/blog.
        ...
        <ul className="mt-5 flex flex-col">
          {headings.map((heading) => (
            <li key={heading.id} className="border-t border-line first:border-t-0">
              <a
                href={`#${heading.id}`}
                className="block py-3 text-sm leading-snug text-slate transition-colors hover:text-brand-dark"
              >
                {heading.text}
              </a>
            </li>
